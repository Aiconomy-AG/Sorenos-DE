...
if (window.BOLD && BOLD.common && BOLD.common.eventEmitter && typeof BOLD.common.eventEmitter.emit === 'function'){
  BOLD.common.eventEmitter.emit('BOLD_COMMON_variant_changed',{variant:variant});
    ...
    _updatePrice: function(variant) {
      ...
      $('.product_price_new').attr('data-price',variant.price);
      $('.price-item.price-item--regular').css('opacity',"0");
      if($('.shappify_qb_grid').length > 0){         
          var actual_price =variant.price;
          var quntity_per=[];
          var quntity_array=[];
          $('.shappify_qb_grid tbody  > tr').each(function(index, tr) { 
          var sd = $(tr).find('td:eq(0)').text().replace(/[^0-9]/gi, '');
          quntity_array.push(sd);
          var per = $(tr).find('td:eq(1)').text().replace(/[^0-9]/gi, '');
          quntity_per[sd]=per;
          });
          var get_per_val = parseInt(quntity_array[0]);
          var get_per = 100 - quntity_per[get_per_val];  
          
          var get_discount_value =actual_price * 100 / get_per;
          $('.product_price_new').attr('data-c-price',get_discount_value.toFixed(2));
          setTimeout(function(){             
            $('.quntity_referace').trigger('click');
            $('.price-item.price-item--regular').css('opacity',"1");
          }, 50);          
      }
      else{
         $('.product_price_new').attr('data-price',variant.price);
...
(typeof window.BOLD !== 'undefined' && typeof window.BOLD.common !== 'undefined' && typeof window.BOLD.common.cartDoctor !== 'undefined' && (response = window.BOLD.common.cartDoctor.fix(response)));
        ...
        function(item, index) {
         $.getJSON('/cart.js', function(cart) {
           if(cart.total_price > 10000){
              ...
              $('[name="checkout"]').val(' Einkaufen');
           }else{
             $('[name="checkout"]').prop('disabled', true);
             var price = cart.total_price / 100;
              $('[name="checkout"]').val('Mindestbestellwert CHF '+ price.toFixed(2));
           }
         });
...
theme.Product = (function() {
  function Product(container) {
    ...
    this.addToCartText = container.querySelector(this.selectors.addToCartText);
    ...
    this.loader = container.querySelector(this.selectors.loader);
  ...
  Product.prototype = Object.assign({}, Product.prototype, {
    ...
    _initAddToCart: function() {
      ...
      productForm.addEventListener(
        ...
        function(evt) {
          ...
          if (note) {
            document.getElementById('Note').setAttribute('name','properties[Hinweis]');
            console.log(note);
          }else {
            document.getElementById('Note').removeAttribute('name');
          }
   ...
   that._setCartQuantity(cart.item_count);
          that._setCartCountBubble(cart.item_count);
});
...
$(document).ready(function () {
  ...
  let quantityProductLabel = $("#SingleOptionSelector-1").val();
  let rollen = "Rollen";
  let meters = "Metern";
  if (quantityProductLabel.includes(rollen)) {
    $('#quantity-product-label').text('Menge')
  }
  if (quantityProductLabel.includes(meters)) {
    $("#quantity-product-label").empty();
    $("#quantity-product-label").append("Meter");
  }
  
  
  $('.site-header__logo-image').removeAttr('data-image-loading-animation');    
  }
...
function changeText(variant) {
  if(variant){
  if(variant.option2 === 'Metern') {
    document.getElementById('quantity-product-label').innerHTML = 'Meter';
  } else {
    document.getElementById('quantity-product-label').innerHTML = 'Menge';
  }
}
}
...
$(document).ready(function($)
{
	...
	var maxHeight = -1;
	$('#shopify-section-160974954273336df1 .feature-row__image').each(function() {
      maxHeight = maxHeight > $(this).height() ? maxHeight :     $(this).height();
    });

    $('#shopify-section-160974954273336df1 .feature-row__image-wrapper').each(function() {
      $(this).height( maxHeight);
    });
  
  	var maxHeight = -1;
	$('#shopify-section-160974954273336df1 .rte-setting').each(function() {
      maxHeight = maxHeight > $(this).height() ? maxHeight :     $(this).height();
    });

    $('#shopify-section-160974954273336df1 .rte-setting').each(function() {
      var heading_height = $(this).siblings('h3').height();
      var remove_heading_height=0;
      
      if(heading_height>22){
        remove_heading_height = 22;
      }
      $(this).height( maxHeight - remove_heading_height );
    });
});

$('.quntity_referace').click(function() {
    if ($('.shappify_qb_grid').length > 0) {
        // console.log('here');
        var quntity_per = [];
        var quntity_array = [];
        var index_array;
        var qutity_value = parseInt($('#Quantity-product-template').val());
        var actual_price = parseInt($('.product_price_new').attr('data-c-price'));
// var actual_price = '3615';
      console.log('qty',qutity_value);
      console.log('actual_price',actual_price);
        $('.shappify_qb_grid tbody > tr').each(function(index, tr) {
            var sd = $(tr).find('td:eq(0)').text().replace(/[^0-9]/gi, '');
            quntity_array.push(sd);
            var per = $(tr).find('td:last()').text().replace(/[^0-9]/gi, '');
            quntity_per[sd] = per;
            // console.log(per);
        });
        var length = quntity_array.length;
      // console.log('length',length)
        if (length > 2) {
            if (qutity_value >= quntity_array[1] && qutity_value < quntity_array[2]) {
                index_array = quntity_array[1];
            } else if (qutity_value >= quntity_array[2]) {
                index_array = quntity_array[2];
            }else{
              index_array = quntity_array[0];
            }
        } else {
            if (qutity_value >= quntity_array[1]) {
                index_array = quntity_array[1];
            }else{
              index_array = quntity_array[0];
            }
        ...
        if (index_array) {
            var get_per = parseInt(quntity_per[index_array]);
            // var get_discount_value = actual_price - (actual_price * get_per) / 100;
          // console.log('get_per', get_per);
            $('.product_price_new').attr('data-price', get_per.toFixed(2));
        }
    }
    var price = parseFloat($('.product_price_new').attr('data-price'));
    var total_quntity = price / 100 * parseInt($('#Quantity-product-template').val());
    var int_part = Math.trunc(total_quntity); // returns 3
    var float_part = Number((total_quntity - int_part).toFixed(2));

    var split = float_part.toString().split('.');

    var lastDigit = Number.isInteger(float_part) ? float_part % 10 : float_part.toString().slice(-1);
    var digit_plus = 0;
    
    // console.log('lastDigit',lastDigit, 'split',split);
    if (split.length == 2) {
        if (lastDigit < 6) {
            // console.log("if");
            if (lastDigit > 0 && lastDigit < 4 ) {
             // console.log("less 4");
                digit_plus = (5 - lastDigit) / 100;
            } else if (lastDigit == 4){
             // console.log("equal 4");
              digit_plus = (4 - lastDigit) / 100;
            } else if (lastDigit > 4){
    ...
    var main_value = (int_part + digit_plus + float_part)*100;
    //console.log('main_value', theme.Currency.formatMoney(main_value, theme.moneyFormat));
    console.log('price',price);
    if(price){
      setTimeout(function(){
         var new_price = theme.Currency.formatMoney(price, theme.moneyFormat);         
         var new_price_html  = `<dt>
        <span class="visually-hidden visually-hidden--inline">Normaler Preis</span>
        </dt><dd><div class="money-details"></div>
         <span class="price-item price-item--regular money pre-money" data-regular-price="" data-product-id="" style="visibility: visible;opacity: 1;">
         ${new_price}
         </span>
         <div class="money-details"></div>
         <span id="exkl-mwst">exkl. MwSt.</span></dd>`;
         console.log('new_price_html',new_price_html);
         $('.product_price_new .price__regular').html(new_price_html);         
      }
      , 100);
    }
...
$(document).ready(function(){
  ...
  $(document).mouseup(function(e) {
    var container = $(".property__field-dropdown");

    // if the target of the click isn't the container nor a descendant of the container
    if (!container.is(e.target) && container.has(e.target).length === 0) 
    {
      container.children('.property__field-list').hide();
    }
  });

  $(".property__field-list input[type='checkbox']").prop('checked', false);
  $('.property__field-inner-text span').remove();
  
  
  $(".property__field-text").on('click', function() {
    $(".property__field-list").slideToggle('fast');
  });
	
  $(".close-dropdown").on('click', function() {
    $(".property__field-list").hide();
  });
  

  $('.property__field-list input[type="checkbox"]').on('click', function() {

    var title = $(this).closest('.property__field-list').find('input[type="checkbox"] + label').val(),
        title = $(this).val();
    
    $('.property__field-inner-text').empty();
    $(".property__field-list input[type='checkbox']").prop('checked', false);
    $(this).prop('checked', true);
    
    if ($(this).is(':checked')) {
      var html = '<span title="' + title + '">' + title + '</span>';
      $('.property__field-inner-text').append(html);
      $(".pre-selected").hide();
    } else {
      $('span[title="' + title + '"]').remove();
      var legchecked = $('.property__field-inner-text span').length;
      if(legchecked == 0){
        $('.pre-selected').show()
      }
    }
...
$(document).ready(function(){
 ...
 $('#ProductSection-product-template .custom-note').click(function(){
    $('.popup-note').show();
    $('.popup-note').addClass('in-open');
    $('.popup-content').addClass('is-active');
    $('body').addClass('note-popup-open');
    $('.custom-note').addClass('popup-open');
    $('.popup-overlay').addClass('popup-open');
  });

  $(".popup-note a.close").click(function(){
    $('.popup-note').hide();
    $('.popup-content').removeClass('is-active');
    $('.popup-note').removeClass('in-open');
    $('body').removeClass('note-popup-open');
    $('.custom-note').removeClass('popup-open');
    $('.popup-overlay').removeClass('popup-open');
  });
});
$('.ajax_submit').click(function(){
  var Hinweis = $('.popup-note #Note').val();  
  if(Hinweis == ''){  
   $('.popup-note #Note').removeAttr( "name" );
  }else {  
   $('.popup-note #Note').attr( "name" , 'properties[Hinweis]' );
  }
  $('[data-add-to-cart]').trigger('click');
  $('.popup-content.is-active .close').trigger('click');
});

 $('.cart__footer').find('.cart__buttons-container').hide();
 $('.cart__footer').find('.else_agree_checkout').show();

$('.agree_checkbox').change(function(){
var $this = $(this);
var check = $(this).find('input[name="agree"]').is(":checked");
if(check){
   $this.parents('.cart__footer').find('.cart__buttons-container').show();
   $this.parents('.cart__footer').find('.else_agree_checkout').hide();
   $('.agreement-msg').hide();  
}
else
{
   $this.parents('.cart__footer').find('.cart__buttons-container').hide();
   $this.parents('.cart__footer').find('.else_agree_checkout').show();
}
});

$('.else_agree_checkout').click(function(){
	$('.agreement-msg').show();
  setTimeout(function(){
	$('.agreement-msg').hide();
  },4000);
...
/*$(window).on('load',function(){
  if($('body').hasClass('werkzeuge')){
  	$('.boost-pfs-filter-option-item').first().find('.boost-pfs-filter-option-value').text('Werkzeuge');
  }
});*/